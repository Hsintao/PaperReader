...
### 产物文件
每次解析会在输出目录中生成：上传的源 PDF、`original.pdf`（原始文件）、译文 PDF、`extraction/manifest.json`（解析清单：页面几何、区块原文与译文、图表与参考文献）以及 `alignment.json`（双语对齐索引）。原文标注 PDF（在原页上框出每个解析区域，包括图注与表注）只在「设置 → 阅读偏好」中开启原文标注后生成；该偏好默认关闭，开启后打开旧文档会按需补建。
