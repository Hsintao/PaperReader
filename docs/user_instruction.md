...
点击左侧栏工具栏最右侧的齿轮按钮打开「设置」，在「模型设置」页填入 API Key 和 Base URL、模型名称，即可用自己的大模型密钥进行翻译。推荐使用价格相对较低的模型，如 DeepSeek-V4-Flash-0731。
...
版式由 worker 负责：它先解析页面结构（分栏、段落、图、表、公式），逐段翻译，再把译文排回原稿页面对应的位置，图片、公式与表格线条保持原位。译文 PDF 是单语版本，不带水印；双语对照由 PaperReader 把原文与译文逐页拼接生成，阅读器直接展示。
...
### 产物文件
每次解析会在输出目录中生成：上传的源 PDF、`original.pdf`（原始文件）、译文 PDF、`extraction/manifest.json`（解析清单：页面几何、区块原文与译文、图表与参考文献）以及 `alignment.json`（双语对齐索引）。原文标注 PDF（在原页上框出每个解析区域，包括图注与表注）只在「设置 → 界面设置」中开启原文标注后生成；该偏好默认关闭，开启后打开旧文档会按需补建。
