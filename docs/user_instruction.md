...
### 产物文件
每次解析会在输出目录中生成：上传的源 PDF、`original.pdf`（原始文件）、译文 PDF、原文标注 PDF（在原页上框出每个解析区域，包括图注与表注）、`extraction/manifest.json`（解析清单：页面几何、区块原文与译文、图表与参考文献）、`extraction/glossary.csv`（翻译器抽取的术语，术语表非空时才有）以及 `alignment.json`（双语对齐索引）。
