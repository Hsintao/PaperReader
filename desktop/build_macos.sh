...
step "Bundle offline translation assets"
# The worker's fonts and layout model otherwise download on first use; the
# offline package (generated by `babeldoc --generate-offline-assets`, see
# release/offline_assets_*.zip) lets a fresh install translate offline. The
# backend restores it on startup; absent the zip, the build is still valid
# and first use downloads as before.
OFFLINE_ZIP="$(ls "$PROJECT_ROOT"/release/offline_assets_*.zip 2>/dev/null | head -n 1 || true)"
if [[ -n "$OFFLINE_ZIP" ]]; then
  mkdir -p "$PROJECT_ROOT/dist/PaperReader.app/Contents/Resources/offline_assets"
