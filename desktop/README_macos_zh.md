# PaperReader v2.1.10 macOS（Apple Silicon）

## 安装与首次启动

1. 下载 `PaperReader-v2.1.10-macOS-arm64.dmg` 及对应 `.sha256`，校验后打开 DMG。
...
## v2.1.10 阅读与 AI 增强并保留 LaTeX 兼容性
...
- 本版本新增：Ctrl/Cmd+F 文档内搜索、可持久化的彩色批注与备注（可导出 Markdown 阅读笔记）、阅读位置记忆、双栏联动滚动、流式 AI 回答与可点击的引用跳转、侧栏跨文档全文搜索、BibTeX 导出、图表墙；PDF 渲染改为按需加载，长文档更流畅。
- PDF 中的 HTTP(S) 外链由系统默认浏览器打开，PaperReader 保持当前论文和阅读位置；PDF 内部章节/页码链接仍在阅读器中跳转。
- 可以拖选、复制 PDF 文字并继续使用双语对应定位；触控板双指捏合缩放更灵敏，目录会优先使用书签，缺少书签时优先使用后端解析出的章节结构。
